...
exports.getLagnaSidereal = getLagnaSidereal;
...
function getJulianDate(year, month, day, hour, minute, timezone = 0) {
    let Y = year;
    let M = month;
    if (M <= 2) {
        Y -= 1;
...
function getHeliocentricCoordinates(planet, T) {
...
function getLunarGeocentricLongitude(T) {
...
function computePlanetPosition(planetName, jd, ayanamsaOffset, lang) {
    const T = (jd - 2451545.0) / 36525.0;
    let tropicalLong = 0;
    let latitude = 0;
    let speed = 1.0;
    ...
    if (planetName === "Sun") {
        if (rasiIdx === 0)
            dignity = "Exalted"; // Aries
        else if (rasiIdx === 6)
            dignity = "Debilitated"; // Libra
        else if (rasiIdx === 4)
            dignity = "Own"; // Leo
    }
    else if (planetName === "Moon") {
        if (rasiIdx === 1)
            dignity = "Exalted"; // Taurus
        else if (rasiIdx === 7)
            dignity = "Debilitated"; // Scorpio
        else if (rasiIdx === 3)
            dignity = "Own"; // Cancer
    }
    else if (planetName === "Mars") {
        if (rasiIdx === 9)
            dignity = "Exalted"; // Capricorn
        else if (rasiIdx === 3)
            dignity = "Debilitated"; // Cancer
        else if (rasiIdx === 0 || rasiIdx === 7)
            dignity = "Own"; // Aries, Scorpio
    }
    else if (planetName === "Mercury") {
        if (rasiIdx === 5)
            dignity = "Exalted"; // Virgo
        else if (rasiIdx === 11)
            dignity = "Debilitated"; // Pisces
        else if (rasiIdx === 2 || rasiIdx === 5)
            dignity = "Own"; // Gemini, Virgo
    }
    else if (planetName === "Jupiter") {
        if (rasiIdx === 3)
            dignity = "Exalted"; // Cancer
        else if (rasiIdx === 9)
            dignity = "Debilitated"; // Capricorn
        else if (rasiIdx === 8 || rasiIdx === 11)
            dignity = "Own"; // Sagittarius, Pisces
    }
    else if (planetName === "Venus") {
        if (rasiIdx === 11)
            dignity = "Exalted"; // Pisces
        else if (rasiIdx === 5)
            dignity = "Debilitated"; // Virgo
        else if (rasiIdx === 1 || rasiIdx === 6)
            dignity = "Own"; // Taurus, Libra
    }
    else if (planetName === "Saturn") {
        if (rasiIdx === 6)
            dignity = "Exalted"; // Libra
        else if (rasiIdx === 0)
            dignity = "Debilitated"; // Aries
        else if (rasiIdx === 9 || rasiIdx === 10)
            dignity = "Own"; // Capricorn, Aquarius
    }
    else if (planetName === "Rahu") {
        if (rasiIdx === 1)
            dignity = "Exalted"; // Taurus
        ...
        if (rasiIdx === 7)
            dignity = "Exalted"; // Scorpio
        else if (rasiIdx === 1)
            dignity = "Debilitated"; // Taurus
        else if (rasiIdx === 0)
            dignity = "Own"; // Aries
    }
    ...
    return {
        name: planetName,
        localizedName: (0, translation_1.translatePlanet)(planetName, lang),
        ...
        signDegree: signDeg,
        rasiIndex: rasiIdx,
        rasiName: (0, translation_1.translateRashi)(rasiIdx, "en"),
        rasiLocalizedName: (0, translation_1.translateRashi)(rasiIdx, lang),
        house: 1, // mapped post-Lagna calculation
        nakshatraIndex: nakIdx,
        nakshatraName: (0, translation_1.translateNakshatra)(nakIdx, "en"),
        nakshatraLocalizedName: (0, translation_1.translateNakshatra)(nakIdx, lang),
        pada: pada,
        navamsaIndex: navIdx,
        navamsaName: (0, translation_1.translateRashi)(navIdx, "en"),
        navamsaLocalizedName: (0, translation_1.translateRashi)(navIdx, lang),
        isRetrograde,
        isCombust,
        dignity,
        strength
...
function getLagnaSidereal(jd, latitude, longitude, ayanamsaOffset) {
    ...
    return codes.map(c => {
        const points = planets.map(p => {
            ...
            if (c.code === "D1") {
                ...
                let startSign = 0;
                if (element === 0)
                    startSign = 0; // Aries starts at Aries
                else if (element === 1)
                    startSign = 9; // Taurus starts at Capricorn
                else if (element === 2)
                    startSign = 6; // Gemini starts at Libra
                else
                    startSign = 3; // Cancer starts at Cancer
            ...
            return {
                ...
                signName: (0, translation_1.translateRashi)(divRasiIdx, "en"),
                signLocalizedName: (0, translation_1.translateRashi)(divRasiIdx, lang),
                house: calculatedHouse
            };
        });
        return {
            name: (0, translation_1.translateDivisionalChartName)(c.code, lang),
            code: c.code,
            description: (0, translation_1.translateDivisionalChartDesc)(c.code, lang),
            points
        };
    });
}
